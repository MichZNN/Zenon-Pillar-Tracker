...
def load_config(config_path: str | Path = BASE_DIR / "config/config.json") -> dict[str, Any]:
    path = _resolve_path(config_path)
    if not path.exists():
        raise FileNotFoundError(
            f"Configuration file not found: {path}. "
            "Copy config/example.config.json to config/config.json first."
        )
    try:
...
class Collector:
    ...
    def _mark_failed(
        ...
        print(f"Collector failed: {message}")
        dev_channel_id = str(
        ...
        if dev_channel_id and self.dispatcher.telegram.enabled:
            try:
                ...
                print(f"Could not send developer error message: {notify_error}")
    ...
    def _update_pinned_message(
        ...
        try:
            ...
            if not self.dispatcher.telegram.response_ok(response):
                print(f"Telegram pinned message returned HTTP {response.status_code}")
        except Exception as exc:
            print(f"Could not update Telegram pinned message: {exc}")
    ...
    def run_forever(self, interval_seconds: int | None = None) -> None:
        ...
        print(f"Collector loop started; polling every {interval} seconds")
        while True:
            try:
                self.run_once()
            except Exception:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--config",
        default=str(BASE_DIR / "config/config.json"),
    ...
    try:
        ...
        print(f"Collector did not complete: {exc}")
        return 1
